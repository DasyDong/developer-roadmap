- [Python基础](#python基础)   
    ...
    - [Python语言基本](#python语言基本)   
        ...
        - [列出 5 个常用 Python 标准库?](#列出-5-个常用-python-标准库)   
        - [Python的内建数据类型有哪些?](#python的内建数据类型有哪些)   
        - [简述 with 方法打开处理文件帮我我们做了什么？](#简述-with-方法打开处理文件帮我我们做了什么)   
        - [Python的可变和不可变数据类型？](#python的可变和不可变数据类型)   
        - [Python 获取当前日期？](#python-获取当前日期)   
        - [谈谈对 Python 的了解和其他语言的区别](#谈谈对-python-的了解和其他语言的区别)   
        - [说说你知道的Python3 和 Python2 之间的区别](#说说你知道的python3-和-python2-之间的区别)   
        - [了解 Python 之禅么？](#了解-python-之禅么)   
        - [了解 docstring 么？](#了解-docstring-么)   
        - [了解类型注解么？](#了解类型注解么)   
        - [例举你知道 Python 对象的命名规范，例如方法或者类等](#例举你知道-python-对象的命名规范例如方法或者类等)   
        - [例举几个规范 Python 代码风格的工具](#例举几个规范-python-代码风格的工具)   
        - [一个编码为 GBK 的字符串 S，要将其转成 UTF-8 编码的字符串，应如何操作？](#一个编码为-gbk-的字符串-s要将其转成-utf-8-编码的字符串应如何操作)   
        - [用正则切分字符串去除非符号](#用正则切分字符串去除非符号)   
        - [单引号、双引号、三引号的区别？](#单引号、双引号、三引号的区别)   
        - [[[1,2],[3,4],[5,6]]一行代码展开该列表，得出[1,2,3,4,5,6]](#[[12][34][56]]一行代码展开该列表得出[123456])   
        - [哪些不能作为字典的健](#哪些不能作为字典的健)   
        - [如何交换字典 {"A"：1,"B"：2}的键和值？](#如何交换字典-"a"：1"b"：2的键和值)   
        - [对生成器类型的对象实现切片功能](#对生成器类型的对象实现切片功能)   
        - [关于list tuple copy 和 deepcopy 的区别是什么？](#关于list-tuple-copy-和-deepcopy-的区别是什么)   
        - [代码中经常遇到的*args, **kwargs 含义及用法。](#代码中经常遇到的args-kwargs-含义及用法)   
        - [Python 中会有函数或成员变量包含单下划线前缀和结尾，和双下划线前缀结尾，区别是什么?](#python-中会有函数或成员变量包含单下划线前缀和结尾和双下划线前缀结尾区别是什么)   
        - [json 序列化时，可以处理的数据类型有哪些？如何定制支持 datetime 类型？](#json-序列化时可以处理的数据类型有哪些如何定制支持-datetime-类型)   
        - [json 序列化时，默认遇到中文会转换成 unicode，如果想要保留中文怎么办？](#json-序列化时默认遇到中文会转换成-unicode如果想要保留中文怎么办)   
        - [如果当前的日期为 20190530，要求写一个函数输出 N 天后的日期，(比如 N 为 2，则输出 20190601)](#如果当前的日期为-20190530要求写一个函数输出-n-天后的日期比如-n-为-2则输出-20190601)   
        - [python 字典和 json 字符串相互转化方法](#python-字典和-json-字符串相互转化方法)   
        - [函数装饰器有什么作用？请列举说明？](#函数装饰器有什么作用请列举说明)   
        - [__call__](#__call__)   
        - [如何判断一个对象是函数还是方法？](#如何判断一个对象是函数还是方法)   
        - [python实现接口 ？](#python实现接口-)   
        - [Python 中的反射了解么?](#python-中的反射了解么)   
        - [metaclass or type](#metaclass-or-type)   
        - [Python中递归的最大次数1000 ?怎么改](#python中递归的最大次数1000-怎么改)   
        - [列举 5 个 Python 中的异常类型以及其含义](#列举-5-个-python-中的异常类型以及其含义)   
        - [w、a+、wb 文件写入模式的区别](#w、a、wb-文件写入模式的区别)   
        - [举例 sort 和 sorted 的区别](#举例-sort-和-sorted-的区别)   
        - [在 requests 模块中，requests.content 和 requests.text 什么区别](#在-requests-模块中requestscontent-和-requeststext-什么区别)   
        ...
        - [字符串 `"123"` 转换成 `123`，不使用内置api，例如 `int()`](#字符串-`"123"`-转换成-`123`不使用内置api例如-`int`)   
        - [Given an array of integers](#given-an-array-of-integers)   
...
- [Python高级](#python高级)   
    - [元类](#元类)   
        - [Python中类方法、类实例方法、静态方法有何区别？](#python中类方法、类实例方法、静态方法有何区别)   
    ...
    - [内存管理与垃圾回收机制](#内存管理与垃圾回收机制)   
        - [哪些操作会导致Python内存泄露，怎么处理？](#哪些操作会导致python内存泄露怎么处理)   
        - [关于内存溢出和内存泄漏的区别](#关于内存溢出和内存泄漏的区别)   
    ...
    - [正则表达式](#正则表达式)   
        ...
        - [a = “abbbccc”，用正则匹配为abccc,不管有多少b，就出现一次？](#a--“abbbccc”用正则匹配为abccc不管有多少b就出现一次)   
        - [Python字符串查找和替换？](#python字符串查找和替换)   
        - [用Python匹配HTML g tag的时候，<.> 和 <.*?> 有什么区别](#用python匹配html-g-tag的时候<>-和-<>-有什么区别)   
    ...
    - [系统编程](#系统编程)   
        ...
        - [说说下面几个概念：同步，异步，阻塞，非阻塞？](#说说下面几个概念：同步异步阻塞非阻塞)   
...
- [Web](#web)   
    ...
    - [Django](#django)   
        - [ORM是什么，ORM的优缺点](#orm是什么orm的优缺点)   
        - [查找 Django 项目中的性能瓶颈](#查找-django-项目中的性能瓶颈)   
        - [什么是wsgi,uwsgi,uWSGI?](#什么是wsgiuwsgiuwsgi)   
        - [Django、Flask、Tornado的对比？](#django、flask、tornado的对比)   
...
- [数据结构](#数据结构)   
    ...
    - [实战](#实战)   
        ...
        - [列表中有n个正整数范围在[0，1000]，进行排序；](#列表中有n个正整数范围在[01000]进行排序；)   
- [大数据](#大数据)   
    - [实战](#实战)   
...
### 关于list tuple copy 和 deepcopy 的区别是什么？
tuple：
```
a = (1, 2, 3, [4, 5, 6, 7], 8)
a[3] = 3  //typeerror
...
### 七层模型？
![七层](http://img.mukewang.com/5b0d085e0001fa2112361724.jpg)
### url的形式？
...
### 基本排序算法汇总(桶排序,归并,快排,希尔,插入,选择,冒泡)
...
#### job执行流程图：
![](https://img-blog.csdnimg.cn/20191205232751459.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzE0ODk4NjEz,size_16,color_FFFFFF,t_70)
