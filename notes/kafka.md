...
* [Kafka](kafka.md#kafka)
  * [常见问题](kafka.md#常见问题)
    * [为什么使用消息队列?消息队列的作用是什么?](kafka.md#为什么使用消息队列消息队列的作用是什么)
    * [Kafka 是依靠什么机制保持高可靠，高可用](kafka.md#kafka-是依靠什么机制保持高可靠高可用)
      * [多副本冗余的高可用机制](kafka.md#多副本冗余的高可用机制)
      * [多个副本之间数据是如何同步的?](kafka.md#多个副本之间数据是如何同步的)
  * [简介](kafka.md#简介)
    * [概述](kafka.md#概述)
    * [消息系统介绍](kafka.md#消息系统介绍)
    * [点对点消息传递](kafka.md#点对点消息传递)
    * [发布-订阅消息传递](kafka.md#发布-订阅消息传递)
    * [Kafka的优点](kafka.md#kafka的优点)
    * [Kafka中的术语解释](kafka.md#kafka中的术语解释)
  * [架构](kafka.md#架构)
    * [架构图](kafka.md#架构图)
      * [分布式模型](kafka.md#分布式模型)
  * [Topics和Partition](kafka.md#topics和partition)
  * [Producer消息路由](kafka.md#producer消息路由)
  * [Consumer Group](kafka.md#consumer-group)
  * [Push vs. Pull](kafka.md#push-vs-pull)
  * [Kafka delivery guarantee](kafka.md#kafka-delivery-guarantee)
...
在 Kafka 集群中，每个 Partition 都有多个副本，其中一个副本叫做 Leader，其他的副本叫做 Follower 多副本之间数据如何同步?
...
在点对点消息系统中，消息持久化到一个队列中。此时，将有一个或多个消费者消费队列中的数据。但是一条消息只能被消费一次。当一个消费者消费了队列中的某条数据之后，该条数据则从消息队列中删除。该模式即使有多个消费者同时消费数据，也能保证数据处理的顺序。这种架构描述示意图如下： ![](../.gitbook/assets/kafka-message.png)

#### 发布-订阅消息传递

在发布-订阅消息系统中，消息被持久化到一个topic中。与点对点消息系统不同的是，消费者可以订阅一个或多个topic，消费者可以消费该topic中所有的数据，同一条数据可以被多个消费者消费，数据被消费后不会立马删除。在发布-订阅消息系统中，消息的生产者称为发布者，消费者称为订阅者。该模式的示例图如下： ![](../.gitbook/assets/kafka-deploy.png)
...
概述 在深入理解Kafka之前，先介绍一下Kafka中的术语。下图展示了Kafka的相关术语以及之间的关系： ![](../.gitbook/assets/kafka-terminology.png)
...
同一个消费组下多个消费者互相协调消费工作，Kafka会将所有的分区平均地分配给所有的消费者实例，这样每个消费者都可以分配到数量均等的分区。Kafka的消费组管理协议会动态地维护消费组的成员列表，当一个新消费者加入消费者组，或者有消费者离开消费组，都会触发再平衡操作。 ![](../.gitbook/assets/kafka-partial.png)
...
![](../.gitbook/assets/kafka-topic.png)
...
Producer发送消息到broker时，会根据Paritition机制选择将其存储到哪一个Partition。如果Partition机制设置合理，所有消息可以均匀分布到不同的Partition里，这样就实现了负载均衡。如果一个Topic对应一个文件，那这个文件所在的机器I/O将会成为这个Topic的性能瓶颈，而有了Partition后，不同的消息可以并行写入不同broker的不同Partition里，极大的提高了吞吐率。可以在$KAFKA\_HOME/config/server.properties中通过配置项num.partitions来指定新建Topic的默认Partition数量，也可在创建Topic时通过参数指定，同时也可以在Topic创建之后通过Kafka提供的工具修改。
...
使用Consumer high level API时，同一Topic的一条消息只能被同一个Consumer Group内的一个Consumer消费，但多个Consumer Group可同时消费这一消息。 ![](../.gitbook/assets/kafka-consumer.png)
